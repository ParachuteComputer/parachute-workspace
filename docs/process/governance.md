...
2. **Version bumps live in release PRs, not feature PRs.** Feature PRs carry code + tests and **do not touch `package.json` version or `CHANGELOG.md`**. A separate release PR bumps the version and writes the changelog covering everything merged since — merging it publishes (see below). Pre-1.0 that bump is the `rc.N` suffix only, keeping `0.X.Y` fixed across the chain; when Aaron says ready-for-release, drop the `-rc` suffix at the same `0.X.Y` to publish `@latest`. Sync the lockfile with every version bump (frozen-lockfile is CI-only). Know the repo's convention — parachute-surface ships stable patch bumps, not rc.
   ...
   **Stable requires an rc that has run on the box first (Aaron, 2026-08-24).** No `next`→`main` release merge until the train has been cut as an rc and has actually run on the operator box (the mini) with an integration pass — deploy from the `next` checkout (bun-linked) or the `@rc` channel, exercise the release's headline surfaces against the live install, and record the pass (what was exercised, at what commit/version) in the release PR before asking for the merge click. Stable-direct trains are retired; this also keeps the `@rc` dist-tag from going stale (the downgrade trap below). Stated after app `0.22.13`/hub `0.7.16` were cut stable-direct with no on-box pass.
