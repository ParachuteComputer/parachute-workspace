...
The **generalist reviewer** (`.claude/agents/reviewer.md`, `subagent_type: "reviewer"`) runs on
**every PR, no exceptions** — even doc-only ones. The **specialists** below each defend ONE
narrow invariant and run *in addition* when the diff touches their domain. This is Lucian's
...
- Generalist: once per PR, **foreground/blocking** (background reviewers starve behind long
  builders). The orchestrator dispatches it — builders can't dispatch reviewers recursively.
...
Spawn a subagent (Agent tool, `reviewer` type) whose prompt = the specialist's brief file below
